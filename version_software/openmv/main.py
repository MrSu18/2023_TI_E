...
while black_rect_flag==0:
    ...
    if rects:
        for r in rects:
            # 判断矩形边长是否符合要求
            if r.w()>40 and r.w()<100 and r.h()>40 and r.h()<100 and r.w()*r.h()>3000:
                 # 在屏幕上框出矩形
                 ...
                 # 矩形会误判的特殊情况处理
                 if abs(corner[1][0]-corner[3][0])<10:
                     if r.w()>70 and r.h()>70:
#                         black_rect_flag=1
                        # 判断识别稳定性
                        if abs(last_corner[0][0]-my_corner[0][0])<5 and abs(last_corner[0][1]-my_corner[0][1])<5:
                           rect_cnt=rect_cnt+1
                        else:
                           rect_cnt=0
                        last_corner=my_corner
                 elif abs(corner[1][1]-corner[3][1])<10:
                     if r.w()>70 and r.h()>70:
#                         black_rect_flag=1
                        # 判断识别稳定性
                        if abs(last_corner[0][0]-my_corner[0][0])<5 and abs(last_corner[0][1]-my_corner[0][1])<5:
